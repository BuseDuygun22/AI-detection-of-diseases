    ...
    def __iter__(self) -> Generator[Tuple[torch.Tensor, torch.Tensor], None, None]:
        ...
        for i in range(0, len(self.indexes), self.batch_size):
            ...
            # If not, we yield a complete batch:
                ...
                x_batch_tensor = torch.cat([x.unsqueeze(0) for x in X_batch], dim=0).float()

                if x_batch_tensor.dim() == 5:
                    x_batch_tensor = x_batch_tensor.squeeze(1)  # Fix extra dimension

                yield x_batch_tensor, torch.tensor(Y_batch).long()
